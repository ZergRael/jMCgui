...
public class SlotData {
	...
	private short damageBlockMetadata;
	...
	public SlotData(TCPReader in) throws Exception
	{
		this.itemBlockId = in.readShort();
		if(this.itemBlockId != -1)
		{
			this.itemCount = in.readByte();
			this.damageBlockMetadata = in.readShort();
			
			for(int i = 0; i < this.enchantableItemsIds.length; i++)
			{
				if(this.enchantableItemsIds[i] == this.itemBlockId)
				{
					short compressedDataLength = in.readShort();
					if(compressedDataLength != -1)
					{
						this.compressedData = new byte[compressedDataLength];
						for(int j = 0; j < compressedDataLength; j++)
							this.compressedData[j] = in.readByte();
					}
					i = this.enchantableItemsIds.length;	
				}
			}
		}
	}
	
	public SlotData() throws Exception
	{
		
	}
	
	public void send(TCPWriter out)
	{
	...
	public short getDamageBlockMetadata() {
		return damageBlockMetadata;
	}
